...
namespace HotUpdate.Game.Battle.Object.Role.Warrior.Projectile
{
    /// <summary>
    /// 战士终结技技能弹射物
    /// </summary>
    public class WarriorUltimateSkillProjectile : InstantProjectile
    {
        ...
        protected override async void CreateVFXOnTrigger()
        {
            foreach (var target in projectileData.targets)
            {
                ...
                await DIContainer.GetInstance<IVFXManager>().CreateVFX(ResKeyCollection.VFX_FirePropertySkill_Hit, projectileTrans, default, newVFXInfo);
                // 计时器计时
