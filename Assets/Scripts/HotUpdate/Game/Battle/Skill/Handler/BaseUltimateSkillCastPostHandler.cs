...
namespace HotUpdate.Game.Battle.Skill.Handler
{
    ...
    public class BaseUltimateSkillCastPostHandler : ISkillCastPostHandler
    {
        ...
        public IEnumerator Handle(SkillContext skillContext)
        {
            ...
            if (context.CurrentCommander != null && currentEntity != context.CurrentCommander)
            {
                // 接着执行其它插队指令，更新UI
                battleController.BattleUiManager.SetCurrentCommanderDisplayUI(context.CurrentCommander);
                battleController.BattleUiManager.RemoveFirstWaitingActUI();
            }
            
            if(CanHandle(context, currentEntity))
            {
                ...
                // TODO：需要获取到指令列表来判断，是否有多个等待的指令，来更新显示
                // 分为多个和单个的情况：多个就显示下一个UI并并从等待列表中移除；单个就显示当前回合角色UI并从等待列表中移除。
        ...
        private static bool CanHandle(IBattleContext context, IBattleEntityObject currentEntity)
        {
