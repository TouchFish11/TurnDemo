...
namespace HotUpdate.Game.Main
{
    ...
    public class SceneGenerator : ISceneGenerator
    {
        ...
        {
            // 销毁相机对象
            Object.Destroy(DIContainer.GetInstance<IOrbitCameraGeter>().OrbitCameraController.Transform.gameObject);
            // 清理玩家数据和对象
            DIContainer.GetInstance<IPlayerManager>().Clear();
            // 清理飘字缓存
            DIContainer.GetInstance<IFloatingTextManager>().ClearCache();
            // 清空对象池
