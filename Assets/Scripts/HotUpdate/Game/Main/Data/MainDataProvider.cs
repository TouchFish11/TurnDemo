...
namespace HotUpdate.Game.Main.Data
{
    ...
    public class MainDataProvider : IMainDataProvider
    {
        ...
        public async Task LoadDataAsync()
        {
            ...
            Logger.Log($"本地音乐数据加载成功，{MusicData}");
            ...
            Logger.Log($"本地输入数据加载成功，{InputActionContainer}");
        ...
        public async Task SaveDataAsync()
        {
            ...
            Logger.Log($"{nameof(MainDataProvider)}.{nameof(SaveDataAsync)}:音乐数据保存成功，{FileUtility.LocalMusicDataFileName}");
            
            // 保存输入数据
            await _binaryDataManager.SaveAsync(FileUtility.LocalInputDataFileName, MainDataCollection.InputActionContainer);
            Logger.Log($"{nameof(MainDataProvider)}.{nameof(SaveDataAsync)}:输入数据保存成功，{FileUtility.LocalInputDataFileName}");
            ...
            Logger.Log($"{nameof(MainDataProvider)}.{nameof(SaveDataAsync)}:游戏设置数据保存成功，{GameSettings}");
        }
        ...
        public void SaveData()
        {
            // 保存音乐数据
             ...
             Logger.Log($"{nameof(MainDataProvider)}.{nameof(SaveData)}:音乐数据保存成功，{FileUtility.LocalMusicDataFileName}");
            
            // 保存输入数据
             _binaryDataManager.Save(FileUtility.LocalInputDataFileName, MainDataCollection.InputActionContainer);
             Logger.Log($"{nameof(MainDataProvider)}.{nameof(SaveData)}:输入数据保存成功，{FileUtility.LocalInputDataFileName}");
