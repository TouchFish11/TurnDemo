...
{
    public class MoveComponentCore : ComponentCore<MoveComponent>
    {
        ...
        // 旋转插值速度（控制转向平滑度）
        private const float rotateSpeed = 15;
        // 旋转阈值（角度）：小于该值时直接对齐目标旋转，停止插值旋转
        private const float rotateThreshold = 5f;
        ...
        protected override void OnInit()
        {
            ...
            characterController = Component.EntityObject.GetComponent<CharacterControllerComponent>().CharacterController;
            // 订阅输入组件的输入变更事件，实时更新输入方向
            Component.EntityObject.GetComponent<InputComponent>().AddKeyInputChangedListener(OnUpdateInputDir);
        ...
        private void UpdateRotate()
        {
            ...
            {
                // 球形插值旋转，保证旋转平滑且速度可控
                _transform.rotation = Quaternion.Slerp(_transform.rotation, targetRot, rotateSpeed * Time.deltaTime);
            }
