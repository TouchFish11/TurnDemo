    ...
    public class HotUpdateEntry : MonoBehaviour
    {
        ...
        private async Task EnterGame()
        {
            try
            {
                ...
                LogManager.Log($"{nameof(HotUpdateEntry)}.{EnterGame()}:Initialization of the AssetBundleManager is complete");
                // 重新初始化UI管理器
                await ServiceLocator.Get<IUIManager>().InitUIManagerAsync(AbKeyCollection.Default, ResKeyCollection.Canvas, ResKeyCollection.UICamera);
                LogManager.Log($"{nameof(HotUpdateEntry)}.{EnterGame()}:Initialization of the UIManager is complete");
                // 初始化场景管理器
                await ServiceLocator.Get<ISceneManager>().InitAsync(AbKeyCollection.Scene);
                LogManager.Log($"{nameof(HotUpdateEntry)}.{EnterGame()}:Initialization of the SceneManager is complete");
                // 初始化输入系统
                await ServiceLocator.Get<IInputSystem>().InitInputsystemAsync(AbKeyCollection.Gameconfig);
                LogManager.Log($"{nameof(HotUpdateEntry)}.{EnterGame()}:Initialization of the InputSystem is complete");
                // 加载热更程序集
                await ServiceLocator.Get<IHotUpdateManager>().LoadAssembliesAsync(AbKeyCollection.Hotupdate);
                LogManager.Log($"{nameof(HotUpdateEntry)}.{EnterGame()}:Load the hotfix assemblies complete");
                // 初始化热更模块管理器
                ...
                LogManager.Log($"{nameof(HotUpdateEntry)}.{EnterGame()}:Initialization of the ModuleManager is complete");
                // 初始化热更工厂
                ServiceLocator.Get<IFactoryManager>().InitHotFactorys();
                LogManager.Log($"{nameof(HotUpdateEntry)}.{EnterGame()}:Initialization of the FactoryManager_HotFactorys is complete");
                // 初始化游戏数据
                await ServiceLocator.Get<IGameManager>().InitDataAsync();
                LogManager.Log($"{nameof(HotUpdateEntry)}.{EnterGame()}:Initialization of the GameData is complete");
                // 加载代理
                LoadMainProxy();
            }
            catch (Exception e)
