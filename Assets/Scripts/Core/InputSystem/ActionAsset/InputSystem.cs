    ...
    public class InputSystem : SingletonBase<InputSystem>, IInputSystem
    {
        ...
        public void EditInput(E_MainActionMap keyMap, Key oldKey, UnityAction<E_KeyConflict> overCallBack)
        {
            ...
            UnityEngine.InputSystem.InputSystem.onAnyButtonPress.CallOnce((inputControl =>
            {
                ...
                // 更新配置容器中的按键映射
                GameDataManager.Instance.InputActionContainer.actionMap[keyMap] = new KeyPathMap(newTempKey, newpath);
                // 刷新输入动作配置
                UpdateActions();
                // 回调：修改完成（无冲突）
        ...
        {
            // 获取输入动作配置容器（存储自定义按键映射）
            var container = GameDataManager.Instance.InputActionContainer;
        ...
        private bool IsKeyConflict(E_MainActionMap oldKeyMap, Key oldKey, Key newKey, string newPath)
        {
            ...
            foreach (var map in GameDataManager.Instance.InputActionContainer.actionMap.Values)
            {
                if (newKey != map.key)
                {
                    continue;
        ...
        private void ExchangeKey()
        {
            var container = GameDataManager.Instance.InputActionContainer;
            // 遍历所有动作映射，找到占用新按键的映射
            foreach (var keyMap in container.actionMap.Keys)
            {
                var keyPathMap = container.actionMap[keyMap];
                ...
                // 缓存原动作映射的配置
                var tempKeyPathMap = container.actionMap[oldKeyMap];
                // 替换原动作映射的按键为新按键
