    ...
    public class ComponentFactory : IFactory
    {
        ...
        public static IEnumerable<(Type, IComponent)> AddComponent(string componentName, IEntityObject entityObject)
        {
            ...
            if (_nameToComponentTypeMap.TryGetValue(componentName, out var componentType))
            {
                ...
                {
                    LogManager.LogWarning($"组件[{componentName}]未实现IComponent接口");
                }
            }
            else
            {
                // 组件名称未注册时记录错误日志，便于问题排查
                LogManager.LogError($"{nameof(AddComponent)}<{componentName}>：未找到该类型的组件");
            }
        }
        ...
        /// </returns>
        public static IEnumerable<IComponent> AddComponent<T>(IEntityObject entityObject) where T : Component, IComponent
        {
