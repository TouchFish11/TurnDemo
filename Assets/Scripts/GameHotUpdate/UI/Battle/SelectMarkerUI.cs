...
namespace GameHotUpdate.UI.Battle
{
    ...
    {
        // 标记旋转速度（每秒旋转角度）
        private const float markerRotationSpeed = 50f;
        // 标记缩放速度
        private const float markerScaleSpeed = 10f;
        // 标记Y轴偏移
        private const float markerYOffset = 50f;
        // 标记的图片UI集合（用于控制标记颜色）
        private readonly List<Image> images = new();
        // 初始旋转角度（用于重置标记旋转）
        private Quaternion originQuaterion;
        // 初始缩放比例
        private readonly Vector3 originScale = Vector3.one * 1.7f;
        // 最终缩放比例
        ...
        private void FollowTarget()
        {
            ...
            UIUtility.WorldToLocalPointInRectangle(
                ...
                Vector2.up * markerYOffset                               // 向上偏移50像素
            );
        }
