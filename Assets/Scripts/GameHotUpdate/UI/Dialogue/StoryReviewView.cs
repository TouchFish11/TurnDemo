...
namespace GameHotUpdate.UI.Dialogue
{
    ...
    /// </summary>
    public class StoryReviewView : BaseUIBehaviour
    {
        /// <summary>
        ...
        protected override async void OnEnable()
        {
            ...
            {
                // 记录启用过程中的异常日志，便于问题定位
                LogManager.LogError($"{GetType().Name}.{nameof(OnEnable)}： {e.Message}");
            }
        }
        ...
        private async System.Threading.Tasks.Task Show()
        {
            ...
            foreach (var dialogueInfo in historicalDialogueInfos)
            {
                ...
                var dialogueReviewUI = await ServiceLocator.Get<IObjectBuilder>().GetHotfixUIObject<DialogueReviewUI>(
                    EAssetBundleType.UI, 
                    ResKeyCollection.DialogueReviewUI, 
                    svReview.content);
                
                // 从二进制数据管理器中获取NPC配置容器，根据说话者ID查询NPC信息
                var npcInfo = ServiceLocator.Get<IBinaryDataManager>()
        ...
        private void ClearReviewUI()
        {
            ...
            {
                ServiceLocator.Get<IPoolManager>().PushObj(dialogueReviewUI.RectTransform.gameObject);
            }
            // 清空UI缓存集合，避免内存泄漏
