...
namespace GameHotUpdate.UI.Task
{
    ...
    public class TaskController : UIController<TaskView, TaskModel>
    {
        ...
        private async Task<TaskTypeContainer> CreateTaskTypeContainer(TaskInfo taskInfo)
        {
            ...
            var taskTypeContainerWrapper = await ServiceLocator.Get<IObjectBuilder>().GetHotfixUIObject<TaskTypeContainer>(EAssetBundleType.UI, ResKeyCollection.TaskTypeContainer, view.TaskContent);
            // 初始化任务类型容器（设置对应的任务类型）
        ...
        private async System.Threading.Tasks.Task CreateTaskItem(TaskInfo taskInfo, TaskTypeContainer container)
        {
            ...
            var taskItemWrapper = await ServiceLocator.Get<IObjectBuilder>().GetHotfixUIObject<TaskItem>(EAssetBundleType.UI, ResKeyCollection.TaskItem, container.transform);
