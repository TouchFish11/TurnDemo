...
namespace GameHotUpdate.Battle.Object.Role.Warrior.Skill
{
    ...
    public class WarriorUltimateSkill : UltimateSkill
    {
        ...
        protected override void InitProjectileAndPoseVfx()
        {
            ...
            // 创建终结技前置POSE特效（展示特效）
            ServiceLocator.Get<IVFXManager>().CreateVFX(ResKeyCollection.VFX_WarriorUltimatePose, projectileTrans, projectileData, vFXInfo);
        }
        ...
        protected override IEnumerator OnUltimateCast(IBattleContext context)
        {
            ...
            // 创建终结技核心攻击特效
            ServiceLocator.Get<IVFXManager>().CreateVFX(ResKeyCollection.VFX_WarriorUltimateSkill, 
