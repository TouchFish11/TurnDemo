...
namespace GameHotUpdate.Battle.Object.Role.Wizard.Skill
{
    ...
    public class WizardUltimateSkill : UltimateSkill
    {
        ...
        /// 构造函数
        /// </summary>
        /// <param name="caster">释放技能的实体（黑塔）</param>
        /// <param name="skillId">技能ID</param>
        /// <param name="statusAddStrategy">状态添加策略</param>
        public WizardUltimateSkill(IBattleEntityObject caster, int skillId, IStatusAddStrategy statusAddStrategy) : base(caster, skillId, statusAddStrategy)
        {
        }
        ...
        protected override void InitProjectileAndPoseVfx()
        {
            ...
            vFXInfo = new VFXInfo();
            // 创建释放姿势的特效
            ServiceLocator.Get<IVFXManager>().CreateVFX(ResKeyCollection.VFX_WizardUltimatePose, projectileTrans, projectileData, vFXInfo);
        }
        ...
        protected override IEnumerator OnUltimateCast(IBattleContext context)
        {
            ...
            // 创建终结技核心特效（命中目标处）
            ServiceLocator.Get<IVFXManager>().CreateVFX(ResKeyCollection.VFX_WizardUltimateSkill, projectileTrans, projectileData, vFXInfo);
