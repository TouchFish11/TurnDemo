...
namespace GameHotUpdate.Battle.Object.Role.Priest.Skill
{
    ...
    public class PriestUltimateSkill : UltimateSkill
    {
        ...
        protected override void InitProjectileAndPoseVfx()
        {
            ...
            //
            ServiceLocator.Get<IVFXManager>().CreateVFX(ResKeyCollection.VFX_Priest_UltimatePose, projectileTrans, projectileData, vFXInfo);
        }
        ...
        protected override IEnumerator OnUltimateCast(IBattleContext context)
        {
            ...
            // 等待动画切换到第一段
            yield return new WaitUntil(() => animationComponent.GetCurrentAnimatorStateInfo(AnimationComponent.Skill_Layer_Name).IsName(Priest_Ultimate_01));
            yield return UpdateCamera_01();
            
            // 等待动画切换到第二段
            yield return new WaitUntil(() => animationComponent.GetCurrentAnimatorStateInfo(AnimationComponent.Skill_Layer_Name).IsName(Priest_Ultimate_02));
        ...
        private void CreateVFX()
        {
            ...
            vFXInfo = ServiceLocator.Get<IPoolManager>().GetData<VFXInfo>();
            // 创建终结技核心特效（命中目标处）
            ServiceLocator.Get<IVFXManager>().CreateVFX(ResKeyCollection.VFX_Priest_UltiamteSkill, projectileTrans, projectileData, vFXInfo);
        }
    }
}
