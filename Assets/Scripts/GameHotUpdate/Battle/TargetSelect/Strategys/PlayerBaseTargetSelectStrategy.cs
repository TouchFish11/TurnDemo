...
namespace GameHotUpdate.Battle.TargetSelect.Strategys
{
    ...
    {
        // 存储筛选后的有效目标列表（复用List避免频繁创建GC）
        private readonly List<IBattleEntityObject> targets = new();
        ...
        /// 选择技能主要目标的核心方法
        /// </summary>
        /// <param name="context">战斗上下文，提供战斗场景内的实体查询能力</param>
        ...
        public IBattleEntityObject SelectMainTarget(IBattleContext context, IBattleEntityObject caster, SkillInfo skillInfo)
        {
            ...
            switch (targetNum)
            {
                // 无有效目标：返回null，技能无法释放
                case 0:
                    LogManager.LogError($"无有效目标:{targetNum}，返回null。技能目标类型：{targetType}，技能信息：{skillInfo.f_id}");
                    return null;
                // 仅有1个有效目标：直接选中该目标
                case 1:
