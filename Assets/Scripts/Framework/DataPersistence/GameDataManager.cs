    ...
    public class GameDataManager : SingletonBase<GameDataManager>, IGameDataManager
    {
        ...
        public async Task InitDataAsync()
        {
            try
            {
                ...
                // 读取音乐数据
                _musicData = BinaryDataManager.Instance.Load<MusicData>(FileUtility.LocalMusicDataFileName);
                if (_musicData == null)
                {
                ...
                // 读取输入动作数据
                _inputActionContainer = BinaryDataManager.Instance.Load<MainActionMapDataContainer>(FileUtility.LocalInputDataFileName);
        ...
        private async Task OnApplicationQuit()
        {
            ...
            if (MusicData != null)
            {
                BinaryDataManager.Instance.Save(FileUtility.LocalMusicDataFileName, _musicData);
            }

            // 保存改键数据
            if (InputActionContainer != null)
            {
                BinaryDataManager.Instance.Save(FileUtility.LocalInputDataFileName, _inputActionContainer);
            }
