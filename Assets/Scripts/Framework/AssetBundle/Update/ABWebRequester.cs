...
public class ABWebRequester : IDisposable
{
    ...
    // 下载进度回调事件
    public event UnityAction<long> OnDownloadProgress;
    ...
    public async void DownLoadAsync(string savePath, UnityAction<bool> overCallback)
    {
        ...
        while (this._request != null && !this._request.isDone && !context.IsPauseDownload)
        {
            // 记录当前下载量
        ...
        else
        {
            ...
            //下载完成, 执行完成回调
            overCallback?.Invoke(this._request?.result == UnityWebRequest.Result.Success);
        }
    }
