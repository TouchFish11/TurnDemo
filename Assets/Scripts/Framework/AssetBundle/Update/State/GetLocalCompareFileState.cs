...
namespace Framework
{
    ...
    {
        public GetLocalCompareFileState(AssetBundleUpdater updater) : base(updater)
        {
        ...
        public async Task<bool> GetLocalCompareFileInfo()
        {
            ...
            if (File.Exists(PathManager.GetAbLoadPath(FileUtility.ListFileDefaultName)))
            {
                ...
                return await GetLocalCompareFileInfo("file:///" + PathManager.GetAbLoadPath(FileUtility.ListFileDefaultName));
            }
...
#endif
                return await GetLocalCompareFileInfo(path + FileUtility.ListFileDefaultName);
            }
            else
            {
        ...
        async Task<bool> GetLocalCompareFileInfo(string localFilePath)
        {
            // 获取本地AB包对比文件
