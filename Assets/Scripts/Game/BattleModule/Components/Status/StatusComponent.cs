...
namespace GameLogic.BattleMoudule.Status
{
    ...
    public class StatusComponent : IStatusComponent
    {
        ...
        public StatusComponent(IBattleEntity owner)
        {
            _owner = owner;
        ...
        private void OnTurnStartHandler(TurnStartEvent turnStartEvent)
        {
            ...
            if (turnStartEvent.CurrentCharacter != _owner)
            {
                return;
            }
            ...
            for (int i = 0; i < _statuses.Count; i++)
            {
                ...
                {
                    _statuses[i].OnTurnStart(_owner, turnStartEvent.Context);
                }
            }
        ...
        public void AddStatus(IStatus status)
        {
            ...
            LogMgr.Log($"{_owner.Name}获得状态：{status.GetType().Name}");
        }
    }
}
