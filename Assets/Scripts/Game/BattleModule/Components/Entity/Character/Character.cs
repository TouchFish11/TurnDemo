...
using GameLogic.BattleMoudule.Talent;
...
{
    /// <summary>
    /// 扩展角色类（包含技能、韧性、追加攻击组件）
    /// </summary>
    public abstract class Character : MonoBehaviour, IBattleEntity
    {
        ...
        public void Init(int id, IBattleContext context)
        {
            // 加载组件（配置表可配置角色绑定哪些组件）
            BattleCharacterComponent battleCharacterComponent = this.gameObject.AddComponent<BattleCharacterComponent>();
        ...
        public void AddRelicBonus(E_RelicBoun type, float value)
        {
