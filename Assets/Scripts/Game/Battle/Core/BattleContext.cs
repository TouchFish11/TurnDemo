...
namespace GameLogic.BattleMoudule.Core
{
    ...
    public class BattleContext : IBattleContext
    {
        ...
        private List<IBattleEntityObject> _allCharacters = new List<IBattleEntityObject>();
        // 回合管理器（核心依赖）
        ...
        /// <returns></returns>
        public async Task CreateEntity(/*object config, int ownerId*/)
        {
            // 批量创建玩家角色（从配置+预制体）
            GameObject playerObj =  await AssetBundleManager.Instance.LoadAssetAsync<GameObject>(E_AssetBundleType.Prefab, "TestPlayer");
        ...
        public void CleanupBattle()
        {
            ...
            {
                Object.Destroy(entity.GetComponent<BattleComponent>().gameObject);
            }
