    ...
    public bool CanCast(IBattleEntityObject caster, ISkill skill)
    {
        switch ((E_SkillType)skill.SkillInfo.f_SkillType)
        {
            ...
            case E_SkillType.CombatSkill:
                ...
                {
                    // 显示提示框UI
                    ServiceLocator.Get<IEventCenter>().TriggerEvent(new GlobalMessageEvent() { Message = "战机点不足，无法释放" });
                    return false;
                }
            case E_SkillType.UltimateSkill:
                ...
                {
                    // 提示玩家能量不足
                    ServiceLocator.Get<IEventCenter>().TriggerEvent(new GlobalMessageEvent() { Message = "能量不足，无法释放终结技" });
                    return false;
                }
